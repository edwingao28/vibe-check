 ...
 */
export function parseRgbFunction(value) {
    // Match rgb(r, g, b) or rgba(r, g, b, a) or rgb(r g b) or rgb(r g b / a)
    const match = value.match(/rgba?\(\s*(\d+(?:\.\d+)?%?)\s*[,\s]\s*(\d+(?:\.\d+)?%?)\s*[,\s]\s*(\d+(?:\.\d+)?%?)/);
    if (!match)
        return null;
 ...
 */
export function parseHslFunction(value) {
    // Match hsl(h, s%, l%) or hsla(h, s%, l%, a) or hsl(h s% l%) or hsl(h s% l% / a)
    const match = value.match(/hsla?\(\s*(\d+(?:\.\d+)?(?:deg|rad|turn)?)\s*[,\s]\s*(\d+(?:\.\d+)?)%\s*[,\s]\s*(\d+(?:\.\d+)?)%/);
    if (!match)
        return null;
 ...
 */
export function resolveTailwindColor(token) {
