 ...
 */
function checkDependencyHealth(needs, extractorHealth) {
    if (needs.length === 0)
        return "all_healthy";
    let allFailed = true;
    let allHealthy = true;
 ...
 */
function makeInsufficientResult(signal) {
    return {
        id: signal.id,
        name: signal.name,
 ...
 */
function reducedConfidence(result, healthStatus) {
 ...
 */
export function runSignals(ctx, config) {
    const results = [];
