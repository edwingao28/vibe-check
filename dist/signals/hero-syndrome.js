 ...
 */
function isHeroSection(fact, allFacts) {
    if (fact.sectionType === "hero")
        return true;
    // Check if this is the first section in its file
    const fileFacts = allFacts.filter((f) => f.file === fact.file);
    if (fileFacts.length > 0 && fileFacts[0] === fact)
        return true;
    return false;
}
/**
 ...
 */
function analyzeWithStructure(structures, texts) {
    ...
    for (const hero of heroSections) {
        ...
        const heroTexts = texts.filter((t) => t.file === hero.file && t.line >= hero.line && t.line <= hero.line + LINE_THRESHOLD);
 ...
 */
function analyzeWithTextOnly(texts) {
    // Group texts by file
 ...
 */
function scoreHeroAnalysis(analysis) {
    ...
    analyze(ctx) {
        ...
        for (let i = 0; i < analyses.length; i++) {
            ...
            if (s > 0) {
                const parts = [];
                if (a.hasHeading)
                    parts.push(`${a.headingCount} heading(s)`);
                if (a.hasParagraph)
                    parts.push(`${a.paragraphCount} paragraph(s)`);
                if (a.hasButton)
                    parts.push(`${a.buttonCount} button(s)`);
                evidence.push({
                    summary: s === 1.0
...
//# sourceMappingURL=hero-syndrome.js.map