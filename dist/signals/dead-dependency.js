 ...
 *   require('framer-motion')
 */
function buildImportPattern(moduleSubstrings) {
    const escaped = moduleSubstrings.map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
    const alternation = escaped.join("|");
    return new RegExp(`(?:from\\s+["'](?:[^"']*(?:${alternation})[^"']*)["']|require\\(\\s*["'](?:[^"']*(?:${alternation})[^"']*)["']\\s*\\))`);
}
/**
...
export const deadDependency = {
    ...
    analyze(ctx) {
        ...
        const installedHeavy = Object.keys(HEAVY_PACKAGES).filter((pkg) => pkgInfo.allDeps.has(pkg));
...
//# sourceMappingURL=dead-dependency.js.map