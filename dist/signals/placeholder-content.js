 ...
 */
import { PLACEHOLDER_CATEGORIES, } from "./data/placeholders.js";
...
function analyzePage(file, texts) {
    ...
    const score = Math.min(1, Math.max(0, totalWeightedHits / SATURATION_THRESHOLD));
            ...
            if (page.score > 0) {
                ...
                for (const cm of page.categoryMatches) {
                    const samples = cm.matchedPatterns
                        ...
                        .map((p) => `"${p}"`)
                        .join(", ");
                    parts.push(`${cm.category.label}: ${cm.matchCount} hit(s) [${samples}]`);
                }
                evidence.push({
...
//# sourceMappingURL=placeholder-content.js.map