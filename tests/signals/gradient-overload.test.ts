...
describe("Gradient Overload signal", () => {
  it("scores high (sloppy) with many gradients per file", () => {
    ...
    const facts = [
      ...
      makeFact({
        property: "background",
        value: "radial-gradient(circle, #667eea, transparent)",
        file: "Hero.tsx",
        line: 9,
      }),
      makeFact({
      ...
      makeFact({
        property: "background",
        value: "linear-gradient(to bottom, #f093fb, #f5576c)",
        file: "CTA.tsx",
        line: 5,
      }),
      makeFact({
        property: "background",
        value: "conic-gradient(red, yellow, lime)",
        file: "CTA.tsx",
        line: 9,
      }),
    ];
  ...
  it("detects various gradient types", () => {
    // Space facts >3 lines apart so each counts as a separate instance
    const facts = [
      makeFact({
        property: "background",
    ...
    // 3 gradient instances in 1 file → avg 3 → score 1.0
