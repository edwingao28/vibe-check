...
interface CardCluster {
  ...
  startLine: number;
  endLine: number;
  hasFeatureGridSection: boolean;
}

interface HeadingParagraphPair {
  headingLine: number;
 ...
 */
function findCardPairs(texts: TextFact[]): HeadingParagraphPair[] {
 ...
 */
function clusterPairs(pairs: HeadingParagraphPair[]): HeadingParagraphPair[][] {
  if (pairs.length === 0) return [];

  const sorted = [...pairs].sort((a, b) => a.headingLine - b.headingLine);
  const clusters: HeadingParagraphPair[][] = [[sorted[0]]];
 ...
 */
function scoreCluster(cardCount: number): number {
  if (cardCount >= 9) return 1.0;
  if (cardCount >= 6) return 0.7;
  if (cardCount >= 4) return 0.5;
  if (cardCount >= 3) return 0.3;
  return 0;
}

export const cardCarnival: SignalDefinition = {
  id: "card-carnival",
  name: "Card Carnival",
  category: "structure",
  ...
  analyze(ctx: SignalContext): SignalResult {
    ...
    const featureGridFiles = new Set(
      structures
