 ...
 */
function detectReplit(fileList: string[]): DetectionHit | null {
  const matched = fileList.filter(
    (f) =>
      /(?:^|\/)replit_integrations\//.test(f) || /(?:^|\/)\.replit$/.test(f),
  );
 ...
 */
function detectBolt(fileList: string[]): DetectionHit | null {
 ...
 */
function detectV0(fileList: string[]): DetectionHit | null {
  const matched = fileList.filter(
    (f) => /(?:^|\/)\.v0\//.test(f) || /(?:^|\/)v0-/.test(f),
  );
  return matched.length > 0
 ...
 */
function detectLovable(fileList: string[]): DetectionHit | null {
 ...
 */
function detectCursor(fileList: string[]): DetectionHit | null {
 ...
 */
function detectGeneratedIcon(fileList: string[]): DetectionHit | null {
  const matched = fileList.filter((f) =>
    /(?:^|\/)generated-icon\.png$/.test(f),
  );
  return matched.length > 0
 ...
 */
function detectExcessiveShadcn(fileList: string[]): DetectionHit | null {
 ...
 */
function detectShadcnTrio(fileList: string[]): DetectionHit | null {
  const hasTailwind = fileList.some((f) =>
    /(?:^|\/)tailwind\.config\.ts$/.test(f),
  );
  const hasPostcss = fileList.some((f) =>
    /(?:^|\/)postcss\.config\.js$/.test(f),
  );
  const hasComponents = fileList.some((f) =>
    /(?:^|\/)components\.json$/.test(f),
  );
  ...
  if (hasTailwind && hasPostcss && hasComponents) {
    ...
    if (tw) matched.push(tw);
    if (pc) matched.push(pc);
    if (cj) matched.push(cj);
    return {
 ...
 */
function detectExcessiveConfig(fileList: string[]): DetectionHit | null {
 ...
 */
function detectDrizzleTemplate(fileList: string[]): DetectionHit | null {
  const drizzleConfigs = fileList.filter((f) =>
...
const ALL_DETECTORS: Array<(fileList: string[]) => DetectionHit | null> = [
  detectReplit,
  detectBolt,
  detectV0,
  detectLovable,
...
export const aiScaffoldSignature: SignalDefinition = {
  ...
  analyze(ctx: SignalContext): SignalResult {
    ...
    // Build evidence
    const evidence: SignalEvidence[] = hits.map((hit) => ({
    ...
    const confidence: "high" | "medium" | "low" =
      platformHits.length > 0 ? "high" : hits.length > 0 ? "medium" : "high";
