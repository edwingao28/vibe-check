 ...
 * Applied to both buttons and links.
 */
function isAppButton(fact: TextFact): boolean {
  const trimmed = fact.text.trim();
  if (trimmed.length === 0) return true;
 ...
 */
function isLikelyMarketingPage(filename: string): boolean {
...
interface FileCtaInfo {
  count: number;
  ctas: TextFact[];
  filteredAppButtons: number;
}
 ...
 */
function countCtasPerFile(texts: TextFact[]): Map<string, FileCtaInfo> {
  const byFile = new Map<string, FileCtaInfo>();
 ...
 */
function scoreForPage(ctaCount: number, filename: string): number {
...
export const ctaMania: SignalDefinition = {
  ...
  analyze(ctx: SignalContext): SignalResult {
    ...
    const marketingPages: [string, FileCtaInfo][] = [];
    const appPages: [string, FileCtaInfo][] = [];
    ...
    const filteredOnlyPages = allPages.filter(
      ([file, { count, filteredAppButtons }]) =>
        filteredAppButtons > 0 &&
