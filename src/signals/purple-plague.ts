...
import type { ColorFact } from "../ir/types.js";
...
function isChromatic(hsl: [number, number, number]): boolean {
  return hsl[1] >= CHROMATIC_SATURATION_THRESHOLD;
}

function isPurple(hue: number): boolean {
...
function analyze(ctx: SignalContext): SignalResult {
  ...
  const resolvedColors = ctx.colors.filter(
    (c): c is ColorFact & { hsl: [number, number, number] } =>
      c.hsl !== undefined && c.confidence !== "low"
  );
  ...
  const rawScore = clamp(
    purpleRatio * 0.6 + (1 - tokenRatio) * 0.2 + lowVariance * 0.2,
    0,
    1
  );
...
export const purplePlague: SignalDefinition = {
  id: "purple-plague",
  name: "Purple Plague",
  category: "typography-color",
  needs: ["css", "tailwind", "inline"],
