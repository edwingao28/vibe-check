 ...
 */
export function generateReport(params: ReportParams): ScanReport {
  const {
    scoringResult,
    irStore,
    scopeResult,
  ...
  if (deepScanSuggested) {
    const tier2Signals = scoringResult.signals.filter(
      (s) =>
        s.id === "hero-syndrome" ||
        s.id === "buzzword-bingo" ||
    ...
    const tier2Fired = tier2Signals.some(
      (s) => s.status === "scored" && s.score > 0.3,
    );
