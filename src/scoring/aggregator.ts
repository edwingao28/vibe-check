...
export function aggregate(
  ...
  config: SlopConfig,
): { slopScore: number; categories: CategoryResult[] } {
  ...
  for (const category of CATEGORIES) {
    ...
    const categorySignals = category.signalIds
      ...
      .filter((s): s is SignalResult => s !== undefined);

    // Count how many are insufficient
    const totalExpected = category.signalIds.length;
    const insufficientCount = categorySignals.filter(
    ...
    const scoredSignals = categorySignals.filter(
      (s) => s.status === "scored",
    );
    ...
    const sumSquared = scoredSignals.reduce(
      (sum, s) => sum + s.attenuatedScore * s.attenuatedScore,
      0,
    );
  ...
  for (const catResult of categoryResults) {
    if (Number.isNaN(catResult.score)) continue;
