...
import postcss, { type Root, type Declaration, type Comment } from "postcss";
...
function getStatus(
  ...
  if (filesAttempted === 0) return "healthy";
  const coverage = filesParsed / filesAttempted;
  if (coverage > 0.95) return "healthy";
  if (coverage >= 0.5) return "degraded";
  return "failed";
}
