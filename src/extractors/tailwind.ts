 ...
 */
function parseTailwindClass(cls: string): ParsedClass | null {
  ...
  const spacingMatch = unprefixed.match(
  ...
  const roundedDirectionMatch = unprefixed.match(
 ...
 */
function parseTailwindColorClass(cls: string): { property: string; token: string } | null {
  const unprefixed = cls.replace(/^(?:\w+:)*/, "");

  // Color classes: text-{color}-{shade}, bg-{color}-{shade}, border-{color}-{shade}, ring-{color}-{shade}
  const colorMatch = unprefixed.match(
    /^(text|bg|border|ring|outline|accent|decoration|fill|stroke)-(.+)$/
  );
  ...
  const nonColorPrefixes: Record<string, string[]> = {
 ...
 */
function extractClassNames(content: string): Array<{ classes: string; line: number }> {
  const results: Array<{ classes: string; line: number }> = [];
 ...
 */
function extractApplyDirectives(content: string, file: string): Array<{ classes: string; line: number }> {
  const results: Array<{ classes: string; line: number }> = [];
  const lines = content.split("\n");
 ...
 */
export function readTailwindConfig(
  projectRoot: string
): { customEntryCount: number; customColors: Record<string, string> } {
  const configFiles = [
    "tailwind.config.js",
    "tailwind.config.ts",
    "tailwind.config.cjs",
 ...
 */
export function parseTailwindFile(
  content: string,
  file: string
): {
  facts: StyleFact[];
 ...
 */
export function extractTailwind(
  files: string[],
  _projectRoot?: string
): ExtractorResult {
